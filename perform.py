...
def find_and_print_matches(query_img_path, threshold=1.0):
    global TP, FP, FN, TN
    ...
    distances = [distance.euclidean(X, feat / np.linalg.norm(feat)) for feat in feats]
...
for subdir, _, files in os.walk(query_folder):
    for file in files:
        if file.lower().endswith(('.png', '.jpg', '.jpeg')):  # Check for valid image files
            ...
            find_and_print_matches(query_img_path, threshold=1.0)  # Adjust threshold as needed
